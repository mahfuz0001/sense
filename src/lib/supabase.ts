...
const createSupabaseClient = (): SupabaseClient => {
  const client = createClient(
    ...
    config.NEXT_PUBLIC_SUPABASE_ANON_KEY,
    {
      ...
      global: {
        headers: {
          "X-Client-Info": `anti-tutorial-hell@${
  ...
  if (isProduction()) {
    ...
    client.from = (relation: string) => {
      ...
      const queryTable = table as unknown as QueryTable;

      ["select", "insert", "update", "delete", "upsert"].forEach((method) => {
        if (typeof queryTable[method] === "function") {
          queryTable[method] = wrapMethod(method, queryTable[method]);
        }
      });
...
export const auth = {
  ...
  getUser: async (accessToken: string) => {
    try {
      ...
      return {
        data: null,
        error: { message: "Failed to get user with token" },
      };
    }
  },
...
export const db = {
  ...
  getUserLearningPathProgress: async (userId: string) => {
    return withRetry(async () => {
      ...
      if (error) {
        logger.error("Failed to get user learning path progress", {
          userId,
          error: error.message,
        });
      }
