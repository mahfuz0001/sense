...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!profile) {
      ...
      const { data: newProfile, error: createError } = await db.createUserProfile(defaultProfile);
    ...
    const { statusCode, apiError } = handleApiError(error, 'Failed to get user profile');
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    // Update user profile
    const { data, error } = await db.updateUserProfile(user.id, validation.data);
    ...
    const { statusCode, apiError } = handleApiError(error, 'Failed to update profile');
