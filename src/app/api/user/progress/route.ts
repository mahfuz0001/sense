...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const totalPoints = completedChallenges * 100 + 
    ...
    const { statusCode, apiError } = handleApiError(error, 'Failed to get user progress');
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const { statusCode, apiError } = handleApiError(error, 'Failed to update progress');
